...
class GameOverScene extends Phaser.Scene {
    ...
    create() {
        ...
        retryText.setInteractive({ useHandCursor: true })
            .on('pointerover', () => {
                console.log('Retry button hover');
                retryText.setStyle({ fill: '#ffff00' });
            })
            .on('pointerout', () => {
                console.log('Retry button out');
        ...
        returnText.setInteractive({ useHandCursor: true })
            .on('pointerover', () => {
                console.log('Return button hover');
                returnText.setStyle({ fill: '#ffff00' });
            })
            .on('pointerout', () => {
                console.log('Return button out');
                returnText.setStyle({ fill: '#ffffff' });
            })
            .on('pointerdown', () => {
                console.log('Return button clicked - Going back to start menu');
                this.scene.stop('GameOverScene');
                this.scene.stop('UIScene');
        ...
        console.log('GameOver UI setup complete');
    }
} 